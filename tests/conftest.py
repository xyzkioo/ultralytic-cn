...
def isolated_model(tmp_path):
    ...
    当多个 xdist worker 同时运行导出测试时，它们会根据模型路径（例如 model.onnx、model.torchscript） 推导输出文件名。使用相同的 MODEL 路径会导致 worker
    相互覆盖中间文件或导出文件。 此 fixture 会将共享模型复制到每个测试独立的临时目录，使每个测试导出到唯一路径。
    """
...
def pytest_sessionfinish(session, exitstatus):
    ...
    仅在 pytest 控制器（或串行运行）上执行，并跳过 xdist worker，避免一个 worker 删除共享资源时另一个 worker 仍在读取而产生竞争条件。
    """
